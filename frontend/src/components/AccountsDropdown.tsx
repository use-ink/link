      ...
      >
        <div className="relative mt-1">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-pink-300 py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {accounts?.map((acc) => (
                <Listbox.Option
                  key={acc.address}
                  className={({ active }) =>
                    `relative cursor-default select-none py-2 pl-10 pr-4 hover:cursor-pointer ${
                      active ? "bg-pink-800 text-gray-300" : "text-gray-300"
                    }`
                  }
                  value={acc}
                >
